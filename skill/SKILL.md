...
1. Read `.tenbo/agent-context.md` (Tier 0 context — load-bearing; regenerated at
   session-end boundaries). If absent (first-encounter recovery only, not steady
   state), fall back to reading `workspace.yaml` + all `roadmap.yaml` +
   `.validation-status.json` and regenerate `agent-context.md` so next session primes
   cleanly. **Explicit-checkpoint trigger:** if the user says "checkpoint",
   "wrap up the session", or similar, regenerate `agent-context.md` immediately.
...
7. **Retroactive done_when sweep.** Pass over `next` + `later` items with cheap
   heuristics (≤2s budget): any recently-done item's id/title keywords appear
   in this item's `done_when` bullets? If candidates exist, surface one line —
   *"N next/later items look like they may be effectively done given recent
   work — want me to walk through them?"* On yes, walk one-by-one (confirm-
   before-flip). If none, stay silent.
...
Before planning or implementing a normal feature/build request, silently refresh
the derived source index, then fetch context:

```bash
npx tenbo-dashboard index --if-stale --json
npx tenbo-dashboard context feature --query "<user request>" --json
```

The index step writes only rebuildable `.tenbo/cache/` data. It is not project memory.
If index refresh is unavailable, slow, or fails, continue to `context feature`; the
context command will fail open with freshness warnings and fallback routing.
...
Use `context.read_plan` as the ordered first-read list before broad source search
unless the request names a specific file. Use `context.verification_plan` to choose
the smallest relevant post-change checks.
Optional context reminders are advisory. They may point agents at
`npx tenbo-dashboard context feature --query "<user request>" --json`, but they must never
block reads, edits, searches, or command execution. Every executable reminder must fail
...
6. **Drift pre-flight.** Load the affected layer's `intent.md` (Anti-responsibilities,
   Invariants, Boundaries, Optimizing-for) and `principles.md` cross-cutting rules; check
   the proposed item against them. If a check trips, surface the *single most relevant*
   violation as decision input, not a block: *"Heads up — this proposes [behavior] which
   violates [layer]'s [rule kind]: '[exact quote]'. Adjust scope, or capture with the
   violation flagged?"* On "capture with violation", record `preflight_violations:
   [{check, outcome: violation, decision: accept-with-violation, rationale}]`. Aligned items pass through silently.
...
2. Triage: **lightweight** (few hours, one layer) → fill `done_when`/`files_to_read`/`risks`
   ...
   **Impact projection (substantial only):** estimate LOC delta, new file count, and new
   item count for each affected layer; compare against `principles.md` thresholds (defaults
   ≥250 LOC added, ≥10 active items per layer). If projected to cross, surface as input —
   *"Completing this would push [layer] from N to ~M (threshold T). Worth landing part in
   a sibling layer?"* User picks scope-adjust / plan-as-is / split; record in `notes:`.
...
npx tenbo-dashboard item brief <id> --json
npx tenbo-dashboard item handoff <id> --json
npx tenbo-dashboard item set-status <id> <now|next|later|done|dropped>
npx tenbo-dashboard item add-note <id> "<note>"
npx tenbo-dashboard item doc-update <id> --date today
npx tenbo-dashboard item complete <id> --evidence "<evidence>" --doc-update today --commit <sha>
npx tenbo-dashboard item verify <id> --check --json
...
npx tenbo-dashboard items --status <status[,status]> --verification <verification-status> --summary --json
npx tenbo-dashboard items --type refactor --status next,later --fields id,title,status,priority,layer,verification --json
npx tenbo-dashboard list --status later --json
npx tenbo-dashboard next --agent-summary --json
npx tenbo-dashboard work-queue --type refactor --status now,next,later --json
npx tenbo-dashboard index --if-stale --json
npx tenbo-dashboard context feature --query "<user request>" --json
npx tenbo-dashboard commit-ready --json
```

Use `item show` for full detail, `items --summary` / `--fields` for compact
agent triage, `item brief` before starting a specific item, `item handoff` when
delegating, and `item complete` for roadmap-only completion hygiene. Use direct
YAML edits only for operations the CLI does not support yet, such as creating a
new item, changing rich fields during planning, or multi-file spec/workpad
archival.
