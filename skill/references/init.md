...
7a. **Ask for product goals.** Before scaffolding `overview.md`, ask once:
    *"Before I write the project overview — what 3 to 6 goals is this project
    optimizing for? (One short sentence each. We can refine later.)"*
    Capture as `g1`/`g2`/`g3`/... (lowercase `g` + integer). Also offer:
    *"Anything explicitly out of scope worth listing as a non-goal?"* (deferrable).
    If the user defers either prompt, scaffold the section with a placeholder
    block per the existing non-interactive convention:
    `<!-- Assumed: goals not yet captured. Refine with the user when convenient. -->`
    Goals belong in `overview.md` only — do NOT add per-layer goals as a separate
    field. Layer-level direction is captured via `Optimizing for` in `intent.md`.
...
10. **Compute health + run init-check + validate (one command).**
    `npx tenbo-dashboard sync` — refreshes the derived source index, runs metrics
    for every scope, then init-check, then validate, in that order. Exits non-zero
    if any step fails. Surfaces new critical/warning findings inline so init can't
    silently produce a half-set-up project. The source index is written under
    `.tenbo/cache/` with a cache-local `.gitignore`, so generated source evidence
    stays out of normal project commits. If this exits non-zero, fix and re-run; do NOT
...
15. **Offer pre-commit hook (sk-032).** Ask once: *"Optional: install a pre-commit
    hook that runs validation on `.tenbo/` before each commit? Catches duplicate IDs,
    broken refs, schema violations before they enter history. Sub-second runtime,
    opt-out anytime via `npx tenbo-dashboard hook uninstall` or `git commit --no-verify`
    per commit."* Accept → run `npx tenbo-dashboard hook install`. Decline → drop, no nag.
...
13. **Offer pre-commit hook (sk-032).** Ask once: *"Optional: install a pre-commit
    hook that runs validation on `.tenbo/` before each commit? Sub-second, opt-out anytime
    via `npx tenbo-dashboard hook uninstall` or `git commit --no-verify` per commit."*
    Accept → run `npx tenbo-dashboard hook install`. Decline → drop, no nag.
...
## Migration: existing projects missing product goals

For projects initialized before product goals existed: on the first session
after the upgrade, if `.tenbo/overview.md` lacks a `## Product goals` section,
the agent offers ONCE per session:

> *"No product goals set yet. Want to define them now? Same setup-time prompt
> — 3 to 6 goals, one sentence each, deferrable."*

Behavior:
- **Accept**: capture goals, insert the `## Product goals` section near the
  top of `overview.md` (after the elevator pitch, before `## Scopes`), and
  optionally prompt for non-goals.
- **Decline**: drop the offer for the rest of the session. Do not nag again
  this conversation. The next session may offer again.
- This check runs at most once per session and is silent (no offer surfaced)
