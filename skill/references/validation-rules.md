...
## Relationship fields (optional `spawned_from:` and `related:`)

Roadmap items MAY include two optional fields that record cross-task relationships
...
Both follow the standard id pattern (`<prefix>-NNN` or `x-NNN`). An item may have
neither, either, or both. Items without these fields render unchanged in the viewer.
...
41. **Relationship id format invalid.** `spawned_from` and every entry in `related`
    must match `^[a-z]{1,5}-\d{3,}$`.
42. **Self-reference.** `spawned_from` must not equal the item's own id; `related`
    must not contain the item's own id.

### Phase-5 warnings

43. **Unknown id reference.** `spawned_from` or any entry in `related` references
    an id that doesn't exist in any scope's roadmap or in the cross-cutting
    roadmap. Warning (not error) because items may be tracked before the
    referenced item is created.
